      ...
      <div class="content">
        <div class="top-area">
          <a-tabs class="nav-links" :active-name="activeName" @tab-click="tabHandleClick">
            <a-tab-pane label="我的组件" name="my"></a-tab-pane>
          </a-tabs>
        ...
        <ul v-if="activeName==='my'" class="content-list">
          <li v-for="item in comLists" class="group-row" :key="item.id">
            ...
            <div class="sourceButton">
              <a-button size="mini" type="primary" class="Button" v-if="userInfo.userId==item.userId" @click="goEditer(item)">编辑</a-button>
              <a-button size="mini" type="danger" class="Button" v-if="userInfo.userId==item.userId&&item.status==1" @click="delet(item,0)">禁用</a-button>
              <a-button size="mini" type="success" class="Button" v-if="userInfo.userId==item.userId&&item.status!=1" @click="delet(item,1)">启用</a-button>
            </div>
          </li>
