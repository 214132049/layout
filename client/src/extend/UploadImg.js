    ...
    init: {
      ...
      FilesAdded (up, files) {
        ...
        if (isAddUploadFile) {
          ...
          Utils.readAsDataURL(files[0].getNative(), function (src) {
            var filesSource = new window.File([Utils.getBlobBydataURI(src, files[0].type)], name, {
              type: files[0].type
            })
...
UploadImage.prototype.send = function (name) {
  ...
  getUploadConfig((configUplaod) => {
    configUplaod.key = configUplaod.dir + name
