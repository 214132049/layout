    ...
    * save (query) {
      try {
        ...
        if (query.id) {
          ...
          if (!(yield this.ctx.model.Category.findOne({where: { status: 1, id: query.id }}))) throw this.ctx.getError({ msg: `不存在id为${query.id}的类型` });
          if (yield this.ctx.model.Category.findOne({where: {name: query.name, status: 1, type: query.type, id: { $ne: query.id }}}))  throw this.ctx.getError({ msg: `已经存在名称为${query.name}的类型` });
          yield this.ctx.model.Category.update(_item, { where: { id: query.id } })
          return { id: query.id }
        } else {
          // 参数验证 是否重名
          if (yield this.ctx.model.Category.findOne({where: {type: query.type, name: query.name, status: 1}}))  throw this.ctx.getError({ msg: `已经存在名称为${query.name}的类型` });
